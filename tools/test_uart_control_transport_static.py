#!/usr/bin/env python3
"""Static checks for the U1 configured-console machine writer contract."""
...
def main() -> int:
    ...
    raw_write = writer.index("uart_write_bytes(kConsoleUart")
    unlock = writer.index("::funlockfile(stdout)")
    assert lock < flush < raw_write < unlock
