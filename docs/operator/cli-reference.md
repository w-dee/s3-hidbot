...
| BLE fixture profiles | `ble-profile-list`, `ble-profile-status`, `ble-profile-select`, `ble-led-status` |
...
| `hidbotctl ble-profile-list` | UART + `ble.fixture-profile-v1` | List the finite development catalog: strict composite, Just Works mouse, keyboard, mouse Report ID 7 and keyboard with LED Output. Availability is not physical qualification. |
| `hidbotctl ble-led-status` | UART + `ble.led-observation-v1` | Observe `{supported, valid, leds}` for the selected LED profile/current connection. The five-bit mask is zero and invalid before a write or after retirement. No physical LEDs are driven. |
| `hidbotctl ble-profile-status` | UART + `ble.fixture-profile-v1` | Show the selected and synchronized active profile, or null active before BLE initialization. |
| `hidbotctl ble-profile-select strict_composite` | UART + `ble.fixture-profile-v1` | Require route-none, ALL_UP, no Sequence/pairing, disconnected hidden BLE, and no transition/fault. Selecting the current profile is a no-op; it does not delete bonds, advertise, or persist a boot setting. |
